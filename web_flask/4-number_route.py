...
- '/number/<int:n>' which returns '<n> is a number', where <n> is an integer.
"""
...
def return_python_text(text):
    """
    Returns 'Python ' followed by the value of the text variable or the
    default 'is cool',with underscores replaced by spaces, when
    accessing the '/python/' or '/python/<text>' path.
    """
    text = text.replace('_', ' ')  # Replace underscores with spaces
...
def number(n):
    """
    Returns '<n> is a number', where <n> is an integer,
